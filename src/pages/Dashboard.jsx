...
const Dashboard = () => {
  ...
  const [newCert, setNewCert] = useState({
    titulo: '', instituicao: '', ano: '', descricao: '', link: ''
  });
  ...
  useEffect(() => {
    ...
    const unsubCerts = onSnapshot(query(collection(db, "certificados"), orderBy("ano", "desc")), (s) =>
      setCerts(s.docs.map(d => ({ id: d.id, ...d.data() }))));

    return () => { unsubCat(); unsubProj(); unsubLogs(); unsubPosts(); unsubCerts(); };
  }, []);
  ...
  const handleSaveCert = async (e) => {
    e.preventDefault();
    if (!newCert.titulo?.trim() || !newCert.instituicao?.trim()) { playErrorSound(); return; }
    playBeep();
    try {
      if (editingCertId) {
        await updateDoc(doc(db, 'certificados', editingCertId), newCert);
        setEditingCertId(null);
      } else {
        await addDoc(collection(db, 'certificados'), newCert);
      }
      setNewCert({ titulo: '', instituicao: '', ano: '', descricao: '', link: '' });
    } catch (e) { playErrorSound(); }
  };
    ...
    <div style={styles.container}>
      ...
      <header style={styles.header}>
        <div style={styles.tabs}>
          ...
          <span onClick={() => { playBeep(); setActiveTab('CERT') }} style={activeTab === 'CERT' ? styles.activeTab : styles.tab}>CERT (CERTIFICADOS)</span>
        </div>
      ...
      <div style={styles.mainScreen}>
        {activeTab === 'STAT' ? (
          ...
          <section>
            <h2 style={styles.sectionTitle}>{editingCertId ? `// EDIT_MODE: ${editingCertId}` : '// CERTIFICATE_PUBLISHER_PROTOCOL'}</h2>
            <div style={styles.grid}>
              <form onSubmit={handleSaveCert} style={styles.formGrid}>
                <input placeholder="CERT_TITLE" value={newCert.titulo} onChange={e => setNewCert({ ...newCert, titulo: e.target.value })} style={styles.input} />
                <input placeholder="INSTITUTION" value={newCert.instituicao} onChange={e => setNewCert({ ...newCert, instituicao: e.target.value })} style={styles.input} />
                <input placeholder="YEAR" value={newCert.ano} onChange={e => setNewCert({ ...newCert, ano: e.target.value })} style={styles.input} />
                <input placeholder="LINK_URL" value={newCert.link} onChange={e => setNewCert({ ...newCert, link: e.target.value })} style={styles.input} />
                <textarea placeholder="DESCRIPTION" value={newCert.descricao} onChange={e => setNewCert({ ...newCert, descricao: e.target.value })} style={{ ...styles.input, gridColumn: 'span 2', height: '80px' }} />
                <div style={{ gridColumn: 'span 2', display: 'flex', gap: '10px' }}>
                  <button type="submit" style={{ ...styles.submitBtn, flex: 3 }}>{editingCertId ? 'COMMIT_EDIT' : 'EXECUTE_CERT_DEPLOY'}</button>
                  {editingCertId && (
                    <button type="button" onClick={() => { setEditingCertId(null); setNewCert({ titulo: '', instituicao: '', ano: '', descricao: '', link: '' }); }} style={{ ...styles.submitBtn, background: '#ff4d4d', flex: 1 }}>ABORT</button>
                  )}
                </div>
              </form>
              <div style={styles.section}>
                <h2 style={styles.sectionTitle}>// REGISTERED_CERTIFICATES</h2>
                <div style={styles.scrollArea}>
                  {certs.map(c => (
                    <div key={c.id} style={styles.projectCard}>
                      <div style={{ flex: 1 }}>
                        <div style={styles.projTitle}>{c.titulo}</div>
                        <div style={styles.projSub}>{c.instituicao} // {c.ano}</div>
                      </div>
                      <div style={{ display: 'flex', gap: '10px' }}>
                        <button onClick={() => { setEditingCertId(c.id); setNewCert(c); playBeep(); }} style={styles.editBtn}>EDIT</button>
                        <button onClick={() => setKillTarget({ colecao: 'certificados', id: c.id, titulo: c.titulo })} style={styles.deleteBtnSmall}>KILL</button>
                      </div>
                    </div>
                  ))}
                </div>
